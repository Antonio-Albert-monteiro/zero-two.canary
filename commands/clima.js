...
exports.run = (client, message, args) => {
	weather.find(
		...
		function(err, result) {
			...
			if (!result)
				return message.reply(`forneça uma cidade. Ex.: \`.clima brasil\``);
			// caso o bot não encontre a cidade
			if (!result[0])
				return message.reply(`desculpe, mas não encontrei essa cidade!`);
...
exports.help = {
  name: "clima",
  aliases: "tempo",
  description: "diz a previsão do tempo para suas cidade ou estado",
  usage: "<prefixo>clima <cidade ou estado>"
}